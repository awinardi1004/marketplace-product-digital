    ...
    <div class="grid grid-cols-4 gap-[22px]">
        @forelse ($other_products as $other_product)
            <div class="product-card flex flex-col rounded-[18px] bg-[#181818] overflow-hidden">
                <a href="{{route('front.details', $other_product->slug)}}" class="thumbnail w-full h-[180px] flex shrink-0 overflow-hidden relative">
                    <img src="{{Storage::url($other_product->cover)}}" class="w-full h-full object-cover" alt="thumbnail">
                    <p class="backdrop-blur bg-black/30 rounded-[4px] p-[4px_8px] absolute top-3 right-[14px] z-10">Rp {{($other_product->price)}}</p>
                </a>
                <div class="p-[10px_14px_12px] h-full flex flex-col justify-between gap-[14px]">
                    <div class="flex flex-col gap-1">
                        <a href="{{route('front.details', $other_product->slug)}}" class="font-semibold line-clamp-2 hover:line-clamp-none">{{$other_product->name}}</a>
                        <p
                            class="bg-[#2A2A2A] font-semibold text-xs text-belibang-grey rounded-[4px] p-[4px_6px] w-fit">{{$other_product->category->name}}</p>
                    </div>
                    <div class="flex items-center gap-[6px]">
                        <div class="w-6 h-6 flex shrink-0 items-center justify-center rounded-full overflow-hidden">
                            <img src="{{Storage::url($other_product->creator->avatar)}}" class="w-full h-full object-cover" alt="logo">
                        </div>
                        <a href="" class="font-semibold text-xs text-belibang-grey">{{$other_product->creator->name}}</a>
                    </div>
                </div>
            </div>
        @empty
                ...
                <div
                    ...
                    <div class="flex gap-[14px] items-center">
                        <div class="w-12 h-12 flex shrink-0 rounded-full overflow-hidden">
                            <img src="{{Storage::url( $product_review->productOrder->buyer->avatar)}}" class="w-full h-full object-cover"
                                alt="photo">
                        </div>
                        <div class="flex flex-col justify-center-center">
                            <p
                                ...
                                {{ $product_review->productOrder->buyer->name }}</p>
                            <p class="font-semibold text-left text-xs text-belibang-grey">{{ $product_review->productOrder->buyer->occupation }}
                            </p>
                        </div>
                    </div>
                </div>
